...
function updateByParent(propertyNode: PropertyNode) {
  ...
  switch (propertyNode.referenceName.toLowerCase()) {
    case "#parent":
      propertyNode.referenceNode = new ReferenceNode(
        propertyNode.node.parent,
        scopeNode.id
      );
      break;
    case "#topparent":
      let topParent = <SceneNode>(
        figma.getNodeById(propertyNode.path[propertyNode.path.length - 2])
      );
      if (topParent) {
...
async function updateAllTextProperty() {
  const searchNodes = figma.currentPage.findAll((node) =>
    /#|_#/.test(node.name)
  );
  ...
  await Promise.all(
    propertyNodes.map((propertyNode) => {
...
export default function () {
  setRelaunchButton(figma.currentPage, "visibleproperty", {
